...
void processImports(ASTNode[] ast, string sourceDir, string targetDir) {
    foreach (node; ast) {
        if (auto imp = cast(ImportNode)node) {
            string localPy = buildPath(sourceDir, imp.modName ~ ".py");
            if (exists(localPy) && (imp.modName !in processedModules)) {
                processedModules[imp.modName] = true;
                string pySrc = readText(localPy);
                auto modLexer = new Lexer(pySrc);
                auto modTokens = modLexer.tokenize();
                auto modParser = new Parser(modTokens);
                auto modAst = modParser.parseProgram();

                processImports(modAst, sourceDir, targetDir);

                string hOutFile = buildPath(targetDir, imp.modName ~ ".h");
                string cOutFile = buildPath(targetDir, imp.modName ~ ".c");

                string hContent = "#ifndef " ~ imp.modName.toUpper() ~ "_H\n#define " ~ imp.modName.toUpper() ~ "_H\n\n";
                foreach (stmt; modAst) {
                    if (auto fn = cast(FunctionDefNode)stmt) {
                        hContent ~= "int " ~ imp.modName ~ "_" ~ fn.name ~ "(";
                        foreach (i, p; fn.params) {
                            hContent ~= "int " ~ p ~ (i < fn.params.length - 1 ? ", " : "");
                        }
                        hContent ~= ");\n";
                    }
                }
                hContent ~= "\n#endif\n";
                std.file.write(hOutFile, hContent);

                auto modGenerator = new CCodegen(imp.modName);
                modGenerator.generate(modAst);
                std.file.write(cOutFile, modGenerator.getSourceCode());
            }
        }
    }
}
...
void runPipeline(string inFile, bool onlyC) {
    ...
    string sourceDir = dirName(absolutePath(inFile));
    ...
    if (onlyC) {
        ...
        try {
            ...
            processImports(ast, sourceDir, currentDir);
    ...
    processImports(ast, sourceDir, srcDir);
