...
class DictNode : ASTNode {
    ...
    override string toC() { return "py_dict_create()"; }
}

class SetNode : ASTNode {
    ASTNode[] elems;
    this(ASTNode[] elems) { this.elems = elems; }
    override string toC() { return "py_set_create()"; }
}
...
class ListCompNode : ASTNode {
    ...
    override string toC() { return "py_list_comp()"; }
}
...
class MethodCallNode : ASTNode {
    ...
    override string toC() {
        if (auto varObj = cast(VarNode)obj) {
            string argsList = "";
            foreach (i, a; args) {
                argsList ~= a.toC() ~ (i < args.length - 1 ? ", " : "");
            }
            return varObj.name ~ "_" ~ method ~ "(" ~ argsList ~ ")";
        }
        if (method == "append") return "py_list_append(&" ~ obj.toC() ~ ", " ~ args[0].toC() ~ ")";
        string res = obj.toC() ~ "_" ~ method ~ "(";
        foreach(i, a; args) {
            res ~= a.toC() ~ (i < args.length - 1 ? ", " : "");
        }
        return res ~ ")";
    }
}
...
class ReturnNode : ASTNode {
    ...
    override string toC() { return "return " ~ (expr ? expr.toC() : "0") ~ ";"; }
}
...
class ImportNode : ASTNode {
    ...
    bool isFromImport;
    this(string modName, string aliasName = "", string[] importedSymbols = [], bool isFromImport = false) {
