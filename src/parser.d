...
class Parser {
    ...
    private ASTNode parseStatement() {
        ...
        if (match(TokenType.KwFrom)) return parseFromImport();
    ...
    private ASTNode parseImport() {
        ...
        if (peek().type == TokenType.Identifier && peek().value == "as") {
            consume();
            aliasName = consume().value;
        }
        return new ImportNode(modName, aliasName);
    }

    private ASTNode parseFromImport() {
        string modName = consume().value;
        if (peek().type == TokenType.Identifier && peek().value == "import") {
            consume();
        } else {
            throw new Exception("Expected 'import' after module name in from-import");
        }
        string[] symbols;
        do {
            symbols ~= consume().value;
        } while (match(TokenType.Comma));
        return new ImportNode(modName, "", symbols, true);
    }
